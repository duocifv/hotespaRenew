(function(DYNns, $, _, undefined) {
    ...
    DYNns.render = {
        get: function(tmplName) {
            ...
            if (_.isUndefined(that.tmplCache[tmplName])) {
                var tmplUrl = (DYNns.System.UNITTEST ? "." : "") + "/common/search/resort_standard/src/" + "dynviews/" + tmplName + ".html";
                var tmplString;
                $.ajax({
                    url: tmplUrl,
                    method: "GET",
    ...
    DYNns.setupSelectDate = {
        get: function(objTab, setday) {
            ...
            var compiled = _.map(that.set, function(setdate) {
                ...
                return _.template("<select class='<%-classname%> style_inputArea  okn e'>" +
                    "<% _.forEach(valueset,function(value){ %>" +
                    "<option value='<%-value%> 1212'><%-value%>88888</option>" +
    ...
    DYNns.setupPersons = function(objTab) {
        ...
        var compiled = _.map(persons, function(person) {
            ...
            return _.template("<%-name%> <li><select class='<%-classname%> style_inputArea'>" +
                "<% _.forEach(values,function(value){ %>" +
                "<option value='<%-value%>' <%if(startPsn===value){%>selected<%}%> ><%-value%>人</option>" +
                "<% })%></select></li><%=personsNumber%><br>")(person);
        });
    ...
    DYNns.setupBudget = {
        get: function(selecter, objBudget, objTab, min0max1num) {
            ...
            var compiled = _.map(arrPrices, function(arrPrice) {
                if (arrPrice) return _.template("<option value='<%-value%>'><%-display%> 5555 </option>")(arrPrice);
            });
            ...
            _.forEach(objTab.sts.category, function(categoryarray, categoryname) {
                if (that.get[categoryname].display(objTab)) {
                    var compiled = _.map(categoryarray, function(category) {
                        if (!_.isUndefined(category)) return _.template("<option value='<%-value%>'><%-name%> 55555</option>")(category);
                    });
    ...
    DYNns.getAirPortJsonp = {
        ...
        set: function(objTab) {
            ...
            _.forEach(that.list[url].airport[0].dep, function(dep, depcount) {
                $(objTab.formid + " .DepartureAirPort").append("<option value='" + dep.code + "'>" + dep.name + "6666</option>");
            });
            _.forEach(that.list[url].airport[0].arr, function(arr, arrcount) {
                $(objTab.formid + " .ArrivalAirPort").append("<option value='" + arr.code + "'>" + arr.name + "7777</option>");
            });
        },
        list: {}
    };
